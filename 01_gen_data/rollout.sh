...
gen_data()
{
	...
	if [[ "$VERSION" == *"gpdb"* ]]; then
		...
		if [ "$VERSION" == "gpdb_6" ]; then
			for i in $(/opt/greenplum-db-6/bin/psql -v ON_ERROR_STOP=1 -q -A -t -c "select row_number() over(), g.hostname, g.datadir from gp_segment_configuration g where g.content >= 0 and g.role = 'p' order by 1, 2, 3"); do
