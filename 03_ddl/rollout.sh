...
for i in $(ls $PWD/*.$filter.*.sql); do
	...
	echo "/opt/greenplum-db-6/bin/psql -v ON_ERROR_STOP=1 -q -a -P pager=off -f $i -v SMALL_STORAGE=\"$SMALL_STORAGE\" -v MEDIUM_STORAGE=\"$MEDIUM_STORAGE\" -v LARGE_STORAGE=\"$LARGE_STORAGE\" -v DISTRIBUTED_BY=\"$DISTRIBUTED_BY\""
	/opt/greenplum-db-6/bin/psql -v ON_ERROR_STOP=1 -q -a -P pager=off -f $i -v SMALL_STORAGE="$SMALL_STORAGE" -v MEDIUM_STORAGE="$MEDIUM_STORAGE" -v LARGE_STORAGE="$LARGE_STORAGE" -v DISTRIBUTED_BY="$DISTRIBUTED_BY"
...
if [ "$filter" == "gpdb" ]; then
	...
	for i in $(ls $PWD/*.ext_tpcds.*.sql); do
		...
		if [ "$VERSION" == "gpdb_6" ]; then
			for x in $(/opt/greenplum-db-6/bin/psql -v ON_ERROR_STOP=1 -q -A -t -c "select rank() over(partition by g.hostname order by g.datadir), g.hostname from gp_segment_configuration g where g.content >= 0 and g.role = 'p' order by g.hostname"); do
		...
		echo "/opt/greenplum-db-6/bin/psql -v ON_ERROR_STOP=1 -q -a -P pager=off -f $i -v LOCATION=\"$LOCATION\""
		/opt/greenplum-db-6/bin/psql -v ON_ERROR_STOP=1 -q -a -P pager=off -f $i -v LOCATION="$LOCATION"
